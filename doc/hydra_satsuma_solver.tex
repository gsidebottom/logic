...
28{,}505\,s of checking. Two instances defeated even the official budget:
\texttt{stone-width3chain-nmarkers-13\_shuffled} (a \emph{62-second}
solve whose SR proof \texttt{dsr-trim} could not check in
45{,}000\,s --- a $>$700$\times$ check-to-solve ratio) and
\texttt{hash\_table\_find\_safety\_size\_21} (157\,s solve,
similarly uncheckable) --- both short solves with disproportionate
unhinted proofs, the study's cleanest exhibits of SR checking cost
(\S\ref{sec:checkercost}).
...
\texttt{satsuma} & 332$^{b}$ & 139 & 193 & 2064 \\
\texttt{hydra\_satsuma} & \textbf{333} & 139 & 194 & \textbf{2037} \\
\bottomrule
\end{tabular}

{\footnotesize $^{a}$\texttt{hydra}'s two \texttt{cake\_lpr}
memouts remain unresolved (their 48\,GB-heap re-check pending; the
same instances' \emph{SR} proofs behaved oppositely --- one verified,
one uncheckable --- and in any case a different chain's certificate
does not certify \texttt{hydra}'s LRAT under competition rules).
$^{b}$satsuma's 16 shared unchecked instances are credited from the
\texttt{hydra\_satsuma}-chain re-checks --- same instance, same
solver family, comparable proofs --- an assumption; its
\texttt{mchess\_17} outcome is its own measurement.
\texttt{stone-width3chain} and \texttt{hash\_table} are counted
\emph{not solved} in both satsuma-chain arms per the rule.}
\end{center}

Under official semantics the structural front-end's margin over the
bare winner is unchanged (+1 solve, now 333 vs.\ 332, with
\texttt{mchess\_20} still the separator), and both satsuma-chain
arms hold a $\sim$16-solve margin over \texttt{hydra} pending its
memout re-check.
