...
We report these results \emph{as if the competition were run on this
machine}: the Apple M4~Pro Mac~mini of \S\ref{sec:checkercost}, with
the satsuma-toolchain components (satsuma, kissat, \texttt{dsr-trim})
executing under Ubuntu~24.04 in Docker containers. All three arms
completed the official 400-slot set at 5000\,s per instance in
...
Soundness was clean across all three runs: zero expected-status
...
\texttt{hydra\_satsuma} & \textbf{335} & 139 & 196 & \textbf{194} & \textbf{2037} \\
\bottomrule
\end{tabular}

{\footnotesize $^{\ast}$competition scoring: SAT instances and
\emph{certified} UNSAT instances contribute their solve time;
uncertified UNSATs and timeouts contribute $2\times 5000$\,s.
satsuma's shared-instance certifications are credited from the
\texttt{hydra\_satsuma}-chain re-checks (same instance, same solver
family, comparable proofs); its \texttt{mchess\_17} outcome is its
own measurement.}
\end{center}

Both runs use the identical protocol on all 400 official slots (the
selection's nine duplicate hashes each occupy two slots and count
twice, as in official scoring).
...
\subsection{Certification economics: the three proof chains}
\label{sec:budgetnote}

The three arms emit certificates through three chains with sharply
...
and SR for the 158 engine verdicts.
...
instances. Its deeper contributions are invariants the count does
not show: instant polynomial certificates on structured families
