...
why local solve counts (e.g.\ the \texttt{hydra} baseline's 318 of
the 391 deduplicated instances) run well above the official 276/400
despite the contention working in the opposite direction. The bare
...
The comparison will report solved counts, PAR-2 under the local
protocol, per-instance and cactus times, and the certification ledger
(verified / unchecked / failed per chain), measuring both what
structure dispatch adds to the winner and what the winner's engine
adds to \texttt{hydra} --- internal comparisons under the protocol
and caveats of \S\ref{sec:benchset-comparability}.
