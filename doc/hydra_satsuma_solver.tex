...
official 2026 benchmark set on this machine.
\end{center}
...
$\sim$47\,GB Linux VM). Per-core, per-second compute on this machine
...
This machine is an Apple M4 Pro (10 performance + 4 efficiency
cores). The competition node model is \emph{assumed}, not confirmed:
...
opposite direction. The bare
\texttt{satsuma} arm of the planned comparison is precisely the
control this argument otherwise lacks: the winner's own pipeline
under our protocol on our hardware, making the hardware factor
directly measurable rather than estimated. One axis runs the \emph{other} way: the competition
...
per instance, certified mode, ten parallel workers, this machine;
