...
Two of the three arms have completed on the official 400-slot set
(5000\,s per instance, certified mode, ten parallel workers, this
machine; satsuma arms in Linux/Docker). The bare \texttt{satsuma}
arm is running; its column will complete the comparison. All numbers
below are internal to the protocol and caveats of
\S\ref{sec:benchset-comparability}.
...
\texttt{satsuma} & \multicolumn{5}{c}{\emph{running}} \\
\bottomrule
\end{tabular}
\end{center}
...
count twice, as in official scoring); per-unique-instance counts in
the head-to-head below differ from slot counts only through those
duplicates. \texttt{hydra}'s two unchecked proofs are
\texttt{cake\_lpr} memouts (\S above), not rejections.

\begin{figure}[h]
\centering
\includegraphics[width=0.86\linewidth]{hydra_satsuma_solver_cactus.pdf}
\caption{Cactus plot on the official 400-slot set (5000\,s, certified
mode, this machine): cumulative instances solved within a given
per-instance time budget; one dot per solved instance.
\texttt{hydra} is projected onto the 400 official slots per hash.
The vertical separation from $\sim$250 instances on is the
satsuma-iter+kissat fall-through's contribution; the bare
\texttt{satsuma} curve will be added when its run completes.}
\label{fig:cactus}
\end{figure}

\textbf{Head-to-head.} On the 391 unique instances common to both
runs there are \emph{zero} contradictory verdicts, \emph{zero}
expected-status mismatches, and \emph{zero} witness failures.
\texttt{hydra\_satsuma} solves 326 unique to \texttt{hydra}'s 311:
eighteen instances only the satsuma arm solves, against three only
\texttt{hydra} solves. The eighteen wins concentrate precisely in
the symmetric families the fall-through exists for: both
\texttt{SC25\_Timetable} instances, the \texttt{lockchart} design
families, \texttt{ramsey\_4\_4\_18}, \texttt{oddball} instances, a
\texttt{le450} graph coloring, and several multiplier-equivalence
UNSATs. The three losses (two \texttt{oddball\_79/80\_5}, one random
CSP SAT) have no structural signature and look like engine variance.
PAR-2 under the local protocol improves 19\%, 2459 $\to$ 1988.

\textbf{Certification ledger} (\texttt{hydra\_satsuma}, 400-slot
run): of 196 UNSAT verdicts, 170 carry machine-verified certificates
--- 27 Cook proofs and 11 GN21 parity proofs (VeriPB, all verified)
plus 132 composed SR proofs (\texttt{dsr-trim} verified in-solver).
The 26 unchecked rows are all satsuma-path UNSATs whose solve
finished (64--3650\,s) but whose \texttt{dsr-trim} pass did not
complete inside the solver's $\sim$5060\,s process wall
(solve\,+\,verify exceeded it; mostly multiplier- and
equivalence-checking circuits with large unhinted proofs, cf.\
\S\ref{sec:checkercost}). None was \emph{rejected}. Under the
competition's separate 45{,}000\,s-per-proof checking budget
(\S\ref{sec:benchset-comparability}) these verifications get
$9\times$ the wall-clock time --- and effectively more, since locally
each received only what remained of the process wall after its solve;
re-checking the 25 unique instances offline is planned.

\textbf{Checker-budget note.} Scores above count solved verdicts;
they are \emph{not} adjusted for local proof-checking timeouts, since
our checking budget (5000\,s) is $9\times$ tighter than the official
45{,}000\,s. The ledger: of \texttt{hydra\_satsuma}'s 196 UNSATs, 26
rows (25 unique instances) hit the local checking budget --- all of
them satsuma-emitted SR proofs, none rejected --- and in fact received
even less than the nominal 5000\,s due to a since-fixed process-wall
bug. We assume these verifications complete within the official
45{,}000\,s: the competition counts an instance only if its proof
checks, SR was an official 2026 checker, and the winner's official
276/400 is therefore by definition net of SR verification at that
budget on competition hardware. The structural certificates never
strain any budget: all 27 Cook and 11 GN21 proofs verified (in
seconds) in both runs, zero timeouts. \texttt{hydra}'s own two
unchecked proofs are \texttt{cake\_lpr} \emph{memouts} under the local
16\,GB heap cap (CaDiCaL-LRAT chain; not rejections). A re-check of
the 25 instances at the official budget is prepared to confirm the
assumption.

\textbf{Reading.} With the engine held to the same structural
front-end, replacing CaDiCaL by satsuma-iter+kissat is worth $+17$
slots and $-19\%$ PAR-2 on this set --- and the win profile
(symmetric design families) matches the fall-through's design intent
rather than generic engine speed. What remains open until the bare
\texttt{satsuma} arm completes: how much of \texttt{hydra\_satsuma}'s
count the winner achieves alone, i.e.\ whether the structural stages
add solves beyond their certified instant verdicts (38 of the 196
UNSATs) or mainly add certification quality and speed.
