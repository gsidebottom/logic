...
53 & \textbf{51.1} & 76 & 1.55 & 10 & \textbf{27} & 4.6\\
\bottomrule
\end{tabular}
\end{center}
...
level 2). At rank 53 the beam's \emph{mean} is $33\times$ the unguided
mean, $5\times$ the unguided 99th percentile, and nearly twice the
unguided \emph{maximum over 5{,}000 draws}; one-shot selection without
guidance (best 1{,}500 of 5{,}000) achieves 4.6, i.e.\ 9\% of the beam
...
beget higher-nearmiss children. \textbf{The gradient is heritable
...
\item \textbf{Fringe-exhaustive} (\texttt{--pursue5 0 --fringe-only}):
  ...
  2-split radius.'' Status at writing: 400/840 parents (48\%),
  \textbf{2.51 billion states explored, zero reductions found}, and an
...
\item \textbf{Gradient chase} (\texttt{--pursue6}): best-first beam on
  ...
  frontier state, closure cap 150). Through level 4 (rank 53):
  nearmiss max 76, beam mean 51.1, with the per-level mean
  \emph{accelerating} ($+9.2$, $+13.6$, $+23.2$) while the unguided
  baseline crawls ($+0.3$, $+0.4$, $+0.6$) --- and the H3 control
  certifies the climb is not rank inflation. Yet through 240 million
  states, \textbf{zero conversions}: 76 one-slot alignments available
  and not one same-partner second-slot event has ever fired. The
  chase asks the one question that matters: \textbf{does nearmiss
  keep climbing until a double-coincidence actually fires, or is
  there an obstruction --- a radius-independent invariant separating
  alignment from reduction?} A conversion produces a genuinely new
  rank-48 scheme (fresh material for the operation-count program of
  the companion papers) or a characteristic-0 rank-47 --- a record. A
