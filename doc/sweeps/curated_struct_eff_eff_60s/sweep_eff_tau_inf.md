# Competition Benchmark Results (index=curated_struct_eff.jsonl, timeout=60s, backend=eff, parallel=10)

## Summary

| Result | Count | % |
|--------|-------|---|
| SAT | 23 | 28.4% |
| UNSAT | 30 | 37.0% |
| TIMEOUT | 28 | 34.6% |
| **Total** | 81 | 100% |

## Cactus plot

![cactus plot](sweep_eff_tau_inf.png)

## Per-problem results

| Problem | Result | Time |
|---------|--------|------|
| bench_10559.smt2 | UNSAT | 0.008s |
| php-010-010.shuffled-as.sat05-1157 | SAT | 0.0008s |
| bench_16516.smt2 | UNSAT | 0.0036s |
| bench_17124.smt2 | UNSAT | 0.0048s |
| bench_11676.smt2 | UNSAT | 0.0132s |
| fphp-010-010.shuffled-as.sat05-1199 | SAT | 0.0015s |
| fphp-012-012.shuffled-as.sat05-1200 | SAT | 0.0022s |
| php-012-012.shuffled-as.sat05-1158 | SAT | 0.0012s |
| urqh2x2.shuffled-as.sat03-1470 | SAT | 0.0007s |
| marg2x2.shuffled-as.sat03-1440 | UNSAT | 0.0007s |
| bench_11496.smt2 | UNSAT | 0.037s |
| hcb2.shuffled-as.sat03-1430 | UNSAT | 0.0006s |
| urqh1c2x2.shuffled-as.sat03-1457 | UNSAT | 0.0026s |
| marg2x3.shuffled-as.sat03-1441 | UNSAT | 0.0062s |
| fclqcolor-08-06-06.shuffled-as.sat05-1265 | SAT | 0.0033s |
| mod2c-3cage-unsat-10-3.sat05-2568.reshuffled-07 | SAT | 0.0735s |
| urqh1c2x3.shuffled-as.sat03-1458 | UNSAT | 46.1696s |
| 3col80_5_8.shuffled | SAT | 0.8217s |
| 3col80_5_5.shuffled | SAT | 0.1254s |
| rope_0001.shuffled | UNSAT | 0.0008s |
| 3col80_5_3.shuffled | SAT | 0.3074s |
| bench_210.smt2 | TIMEOUT | 60s |
| bench_5712.smt2 | TIMEOUT | 60s |
| bench_246.smt2 | TIMEOUT | 60s |
| bench_14437.smt2 | TIMEOUT | 60s |
| bench_1604.smt2 | TIMEOUT | 60s |
| ph12 | TIMEOUT | 60s |
| mod2-rand3bip-sat-200-3.shuffled-as.sat05-2145 | TIMEOUT | 60s |
| fphp-010-008.shuffled-as.sat05-1213 | TIMEOUT | 60s |
| clqcolor-08-06-06.shuffled-as.sat05-1241 | TIMEOUT | 60s |
| 3col20_5_8.shuffled | UNSAT | 0.001s |
| 3col20_5_6.shuffled | UNSAT | 0.0012s |
| 3col20_5_1.shuffled | UNSAT | 0.001s |
| 3col20_5_7.shuffled | UNSAT | 0.0012s |
| Break_04_06.xml | SAT | 0.0156s |
| Break_triple_04_06.xml | SAT | 0.0172s |
| Break_04_08.xml | SAT | 0.014s |
| x9-02007.sat.sanitized | SAT | 0.002s |
| Break_triple_04_08.xml | SAT | 0.0168s |
| x9-02023.sat.sanitized | SAT | 0.0056s |
| Break_04_10.xml | SAT | 0.0152s |
| x9-02044.sat.sanitized | SAT | 0.0034s |
| Break_triple_04_10.xml | SAT | 0.02s |
| x9-02090.sat.sanitized | SAT | 0.0054s |
| x9-02043.sat.sanitized | SAT | 0.0015s |
| x9-02036.sat.sanitized | UNSAT | 0.0061s |
| x9-02042.sat.sanitized | UNSAT | 0.0076s |
| x9-02021.sat.sanitized | UNSAT | 0.0057s |
| x9-02053.sat.sanitized | UNSAT | 0.0089s |
| x9-02073.sat.sanitized | UNSAT | 0.0078s |
| Break_triple_04_04.xml | SAT | 0.0869s |
| Break_unsat_04_03.xml | UNSAT | 0.1306s |
| Break_04_04.xml | SAT | 0.2031s |
| ezfact16_8.shuffled | UNSAT | 0.4413s |
| ezfact16_7.shuffled | UNSAT | 0.7747s |
| ezfact16_5.shuffled | UNSAT | 1.0464s |
| ezfact16_10.shuffled | UNSAT | 2.2922s |
| ph9 | UNSAT | 1.9899s |
| x1_16.shuffled | UNSAT | 0.1921s |
| x2_16.shuffled | UNSAT | 0.2514s |
| ezfact16_6.shuffled | UNSAT | 31.4176s |
| php-010-008.shuffled-as.sat05-1171 | TIMEOUT | 60s |
| Break_unsat_06_07.xml | TIMEOUT | 60s |
| ezfact32_2.shuffled | TIMEOUT | 60s |
| ezfact32_4.shuffled | TIMEOUT | 60s |
| ezfact32_9.shuffled | TIMEOUT | 60s |
| fphp-010-009.shuffled-as.sat05-1227 | TIMEOUT | 60s |
| mod2-rand3bip-sat-200-2.shuffled-as.sat05-2144 | TIMEOUT | 60s |
| php-010-009.shuffled-as.sat05-1185 | TIMEOUT | 60s |
| VanDerWaerden_pd_2-3-18_311 | TIMEOUT | 60s |
| x1_24.shuffled | UNSAT | 40.1732s |
| x2_24.shuffled | TIMEOUT | 60s |
| x2_32.shuffled | TIMEOUT | 60s |
| x1_32.shuffled | TIMEOUT | 60s |
| VanDerWaerden_pd_2-3-18_313 | TIMEOUT | 60s |
| 38bits_10.dimacs | TIMEOUT | 60s |
| 40bits_10.dimacs | TIMEOUT | 60s |
| mod2-rand3bip-sat-200-1.shuffled-as.sat05-2143 | TIMEOUT | 60s |
| VanDerWaerden_pd_2-3-18_300 | TIMEOUT | 60s |
| VanDerWaerden_pd_2-3-18_298 | TIMEOUT | 60s |
| x1_36.shuffled-as.sat03-1589 | TIMEOUT | 60s |
