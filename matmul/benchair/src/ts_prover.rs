...
/// `setup_preprocessed` with relaxed bounds (BaseAir only): the
/// upstream version requires `Air<p3_uni_stark::ProverConstraintFolder>`,
/// which two-stage AIRs (implemented against our forked folder) do
/// not satisfy. Body is identical.
pub fn ts_setup_preprocessed<SC, A>(
    config: &SC,
    air: &A,
    degree_bits: usize,
) -> Option<(
    PreprocessedProverData<SC>,
    p3_uni_stark::PreprocessedVerifierKey<SC>,
)>
where
    SC: StarkGenericConfig,
    A: p3_air::BaseAir<Val<SC>>,
{
    let pcs = config.pcs();
    ...
    let vk = p3_uni_stark::PreprocessedVerifierKey {
        width,
        degree_bits,
        commitment,
    };
