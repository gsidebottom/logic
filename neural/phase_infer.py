...
def main() -> None:
    ...
    logits = model(graph_to_inst(g))
    phases = np.array(mx.sigmoid(logits) >= 0.5)        # (n_vars,) bool
    dt = time.time() - t0

    lits = [(v + 1) if phases[v] else -(v + 1) for v in range(g.n_vars)]
    out = sys.stdout if args.out == "-" else open(args.out, "w")
    out.write(f"c phase predictions: {g.n_vars} vars, {g.n_clauses} clauses, "
              f"{int(phases.sum())} True ({dt*1000:.0f} ms)\n")
