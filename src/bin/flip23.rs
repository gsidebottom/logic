//! Rational (Z[1/2]) flip-graph exploration for 3x3 matmul, seeded at
//! rank-23 record schemes (default seed: our 55-add scheme, mm23).
//! Port of flip48 to <3x3x3>: 9-coordinate factors, 23 summands,
//! 729-equation exact verification.  Known bounds: 19 <= R(3x3) <= 23;
//! any verified rank <= 22 here is a WORLD RECORD.
//!
//! Unlike the DPS-48 seed (flip-isolated), rank-23 3x3 seeds are
//! NATIVELY mobile: the 55-add seed has 9 proportional factor pairs
//! (3 per slot), so solved flips exist from move one.
...
//!           (rank −1).  At rank <= 22: dump, verify, scream.
//!
...
impl Vec9 {
    ...
    fn canon(&self) -> (Vec9, bool, i32, i64) {
    ...
    fn add_scaled(&self, other: &Vec9, neg: bool, k: i32) -> Option<Vec9> {
        let oe = other.exp + k;
        ...
        Some(Vec9 { nums, exp: e }.normalize())
    }
    fn max_abs(&self) -> i64 {
...
#[derive(Clone)]
struct Summand {
    a: Vec9, // primitive, sign-normalized
    b: Vec9, // primitive, sign-normalized
    c: Vec9, // carries the dyadic scalar (any dyadic vector)
}
...
fn try_split(
    ...
    let mut part = Vec9 { nums: fk.nums, exp: fk.exp + m };
    if neg {
    ...
    for i in 0..n {
        for j in i + 1..n {
            ...
            let merged: Option<Summand> = if si.a == sj.a && si.b == sj.b {
                ...
                if ci == cj {
                    let mk = |v: &Vec9, e: i32, g: i64| -> Option<Vec9> {
                        let mut w = v.clone();
                        w.exp += e;
...
/// i.e. solve  f_i + lam f_j = mu f_m  (2 unknowns, 9 equations),
...
fn coincidence_lams(
    ...
    for &t1 in &others {
        ...
        for m in 0..scheme.len() {
            ...
            // pivot coordinates, then verify all 9 exactly.
            ...
            if (0..9).all(|x| det * gi(x) + nl * gj(x) - nm * gm(x) == 0) {
                out.push((t1, lam, m));
            }
        }
    }
...
fn scheme_hash(scheme: &[Summand]) -> u64 {
    ...
    let mut keys: Vec<(&Vec9, &Vec9, &Vec9)> =
...
/// Every hit is a constructively-derived rank-23 scheme (or lower!);
...
fn pursue(seed: &[Summand], cap: i64, outdir: &str) {
    ...
    let mut new23: HashSet<u64> = HashSet::new();
    let mut hits = 0u64;
    let mut alarms = 0u64;
    let mut saved = 0u32;
    ...
    println!(
        "pursue: {} double-coplanar executions, {} NEW distinct rank-23          schemes ({} saved), {} sub-23 alarms",
        hits, new23.len(), saved, alarms
    );
}
...
fn pursue3(seed: &[Summand], cap: i64, depth_max: u32, outdir: &str) {
    ...
    let roots: Vec<(usize, usize, usize)> = (0..3)
        .flat_map(|s| {
            (0..23).flat_map(move |i| {
                (0..23).filter(move |&k| k != i).map(move |k| (s, i, k))
            })
        })
        .collect();
        ...
        while let Some((state, depth)) = stack.pop() {
            ...
            if r.len() < 23 {
                ...
                if h != seed_hash && verify(&r) {
                    ...
                    if a < 40 {
                        let p = format!("{outdir}/new23_p3_{}.txt", a);
                        dump(&r, &p);
                        println!("NEW rank-23 scheme (pursue3) -> {p}");
                    }
                }
            }
            if depth >= depth_max {
    ...
    println!(
        "pursue3(depth {}): {} nodes explored, {} NEW rank-23, {} sub-23 alarms",
        depth_max,
...
fn closure(
    ...
    while let Some(state) = stack.pop() {
        ...
        if reduced {
            if r.len() < 23 {
                if verify(&r) {
                    let k = n_sub23.fetch_add(1, Ordering::Relaxed);
                ...
                if rh != seed_hash && verify(&r) {
                    let k = n_new23.fetch_add(1, Ordering::Relaxed);
                    if saved.load(Ordering::Relaxed) < 40 {
                        saved.fetch_add(1, Ordering::Relaxed);
                        let p = format!("{outdir}/new23_p4_{}.txt", k);
                        dump(&r, &p);
                        println!("NEW rank-23 (pursue4) -> {p}");
                    }
                }
            }
            // continue exploring from the reduced state
            if r.len() >= 23 && seen.len() < max_seen {
                stack.push(r);
            }
        }
...
/// (reductions are first-class moves — the 25 -> 24 -> 23 path);
...
fn pursue4(seed: &[Summand], cap: i64, splits: u32, outdir: &str) {
    ...
    let n_new23 = AtomicU64::new(0);
    let n_sub23 = AtomicU64::new(0);
                ...
                for i in 0..n {
                    for k in 0..n {
                        ...
                        let cl = closure(base, seed_hash, cap, 20_000, outdir,
                                         n_states, n_new23, n_sub23, saved);
    ...
    println!(
        "pursue4(splits {}): {} states, {} NEW rank-23, {} sub-23 alarms",
        splits,
        n_states.load(Ordering::Relaxed),
        n_new23.load(Ordering::Relaxed),
        n_sub23.load(Ordering::Relaxed)
    );
}
        ...
        for y in x + 1..n {
            for z in y + 1..n {
                let e = fac(&st[x], 0).exp
                    .min(fac(&st[y], 0).exp)
                    .min(fac(&st[z], 0).exp);
                let g = |v: &Vec9, k: usize| -> i128 {
                    (v.nums[k] as i128) << (v.exp - e) as u32
                };
                let (vx, vy, vz) =
                    (fac(&st[x], 0), fac(&st[y], 0), fac(&st[z], 0));
                // coplanar iff all 3x3 minors over the 9 coords vanish
                let mut ok = true;
                'mn: for p in 0..9 {
                    for q in (p + 1)..9 {
                        for r in (q + 1)..9 {
...
/// dyadic Vec9 as a sparse combination of 4x4 matrix units e_rc
fn pretty_v16(v: &Vec9) -> String {
...
fn worked(seed: &[Summand], cap: i64) {
    ...
    println!("SEED  rank 23  shared {sh}  coinc {co}  nearmiss {nm}  copl {cp}  maxc {mx}");
    ...
    'outer: for ss in 0..3usize {
        for i in 0..23usize {
            for k in 0..23usize {
                ...
                println!("  new summand 23 carries the remainder factor: {}",
                         pretty_v16(fac(&root[23], ss)));
                let (sh, co, cp, mx, nm) = pm(&root);
                println!("  ROOT  rank 24  shared {sh}  coinc {co}  nearmiss {nm}  copl {cp}  maxc {mx}");
                for line in walk {
                    println!("{line}");
                }
...
/// samples splits), then nearmiss_of the raw endpoint (rank 23+k;
...
fn nmrand(seed: &[Summand], cap: i64, maxk: usize, n: usize) {
    ...
    for k in 1..=maxk {
        ...
        let cnt = nms.len();
        if cnt == 0 {
            println!("{k:>3} {:>5} {:>6} (no completed walks)", 23 + k, k - 1);
            continue;
        }
        ...
        println!(
            ...
            23 + k, k as i64 - 1, q(0.5), q(0.9), q(0.99),
            nms[cnt - 1]
        );
    }
}
...
fn pursue5(seed: &[Summand], cap: i64, k_sample: usize, budget: u64,
    ...
    println!(
        "pursue5 done: {} states, {} NEW rank-23, {} sub-23, max depth-2 nearmiss {}",
        n_states.load(Ordering::Relaxed),
        n_new23.load(Ordering::Relaxed),
        n_sub23.load(Ordering::Relaxed),
        max_nm2.load(Ordering::Relaxed)
...
/// parse one dumped Vec9: "([n1, n2, ...], exp)"
fn parse_v16(s: &str) -> Option<Vec9> {
    ...
    Some(Vec9 { nums, exp: r.trim().parse().ok()? })
}
...
fn pursue6(seed: &[Summand], cap: i64, beam: usize, k_sample: usize,
    ...
    for level in (start_level + 1)..=depth {
        ...
        println!(
            "chase level {}: frontier {}  nearmiss max {}  mean {:.1}               states {}  new23 {}  sub23 {}",
    ...
    println!(
        "chase done: {} states, {} NEW rank-23, {} sub-23",
        n_states.load(Ordering::Relaxed),
        n_new23.load(Ordering::Relaxed),
        n_sub23.load(Ordering::Relaxed)
    );
...
fn main() {
    ...
    println!("seed loaded + exactly verified (23 summands)");
    ...
    if let Some(pi) = args.iter().position(|a| a == "--graph") {
        let path = args.get(pi + 1).cloned()
            .unwrap_or_else(|| "graph27.json".into());
    ...
    if args.iter().any(|a| a == "--census") {
        let (sh, co, cp, mx, nm) = state_metrics(&seed.to_vec());
        println!("seed metrics: shared {sh}  coinc {co}  nearmiss {nm}  copl {cp}  maxc {mx}");
        for ss in 0..3usize {
            for x in 0..seed.len() {
                for y in x + 1..seed.len() {
                    if fac(&seed[x], ss) == fac(&seed[y], ss) {
                        println!("  shared {}-factor pair ({x},{y}): {}",
                                 SLOT[ss], pretty_v16(fac(&seed[x], ss)));
                    }
                }
            }
        }
    ...
    if args.iter().any(|a| a == "--native") {
        // close the SEED itself under solved flips at rank 23 — no
        // splits.  Any reduction found here is rank 22: a world record.
        let maxn = get("--max", 2_000_000) as usize;
        let n_states = AtomicU64::new(0);
        let n_new = AtomicU64::new(0);
        let n_sub = AtomicU64::new(0);
        let saved = AtomicU64::new(0);
        let t0 = Instant::now();
        let comp = closure(seed.clone(), scheme_hash(&seed), cap, maxn,
                           &outdir, &n_states, &n_new, &n_sub, &saved);
        println!(
            "native component: {} states in {:.1}s (cap {maxn}){}",
            comp.len(),
            t0.elapsed().as_secs_f64(),
            if comp.len() >= maxn { "  [TRUNCATED at cap]" } else { "" }
        );
        println!("REDUCTIONS: sub-23 {}  re-landed-23 {}  saved {}",
                 n_sub.load(Ordering::Relaxed),
                 n_new.load(Ordering::Relaxed),
                 saved.load(Ordering::Relaxed));
        let step = (comp.len() / 4000).max(1);
        let mut nmh = std::collections::BTreeMap::new();
        let mut sinks = 0usize;
        for st in comp.iter().step_by(step) {
            let (_, co, _, _, nm) = state_metrics(st);
            *nmh.entry(nm).or_insert(0usize) += 1;
            if co == 0 {
                sinks += 1;
            }
        }
        println!("nearmiss histogram (every {step}th state): {nmh:?}  sinks {sinks}");
        return;
    }
    if args.iter().any(|a| a == "--lams") {
        // exhaustive BFS over the GENERAL flip graph: every shared pair,
        // both orientations, all lam in LAMS (no coincidence required),
        // reduction probe at every state.  The 3x3 seed has 9 eligible
        // pairs but ZERO solved coincidences, so this — not the
        // solved-move closure — carries the native mobility at rank 23.
        let maxn = get("--max", 50_000) as usize;
        ...
        let (mut n_states, mut n_red, mut sinks, mut coinc_pos) = (0usize, 0usize, 0usize, 0usize);
        while let Some(st) = q.pop_front() {
            n_states += 1;
            // reduction probe: any landing below 23 is a world record
            let mut r = st.clone();
            let mut red = false;
            while try_reduce(&mut r, cap * 4) {
                red = true;
            ...
            if red && r.len() < 23 && verify(&r) {
                n_red += 1;
                println!("!!! RANK {} SCHEME — WORLD RECORD — verified !!!", r.len());
                dump(&r, &format!("{outdir}/RECORD_rank{}_{}.txt", r.len(), n_red));
            }
            let n = st.len();
            let (mut any_pair, mut any_coinc) = (false, false);
            for s2 in 0..3usize {
                for x in 0..n {
                    for y in x + 1..n {
                        if fac(&st[x], s2) != fac(&st[y], s2) {
                        ...
                        any_pair = true;
                        if !any_coinc
                            && (!coincidence_lams(&st, x, y, s2).is_empty()
                                || !coincidence_lams(&st, y, x, s2).is_empty())
                        {
                            any_coinc = true;
                        }
                        if seen.len() >= maxn {
                        ...
                        for &(oi, oj) in &[(x, y), (y, x)] {
                            for &lam in LAMS.iter() {
                                let mut c = st.clone();
                                if !try_flip(&mut c, oi, oj, s2, lam, cap) {
                                    continue;
                                }
                                let h = scheme_hash(&c);
                                if seen.insert(h) {
            ...
            if n_states % 25_000 == 0 {
                println!(
                    "[lams] visited {n_states}  frontier {}  seen {}  red {n_red}  {:.0}s",
                    q.len(), seen.len(), t0.elapsed().as_secs_f64()
                );
            }
        }
        println!(
            "lams component: visited {n_states} (seen {}{})  {:.1}s",
            seen.len(),
            if seen.len() >= maxn { ", TRUNCATED at cap" } else { ", complete" },
            t0.elapsed().as_secs_f64()
        );
        println!(
            "reductions below 23: {n_red}   flip-ineligible sinks: {sinks}   states with ANY solved coincidence: {coinc_pos}"
        );
        return;
    }
